...
**v1 (backend + frontend) is functionally complete end-to-end and on GitHub.** Remaining open items: decide on a free-tier Postgres host (Neon/Supabase) for anything beyond local Docker, and optionally a real Alembic baseline migration once that host exists.
