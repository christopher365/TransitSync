...
10. 🔄 Build a frontend dashboard — NOT part of the original planning session's roadmap (steps 1-9 only ever covered the backend); added 2026-08-26 once the backend was working end-to-end and there was no way to see it. React + Vite + react-leaflet (OpenStreetMap tiles, no API key/cost), served via nginx in its own Docker service. See "Frontend" in `docs/architecture-decisions.md` for the trade-off (a plain HTML/JS page was the simpler option but a full React split was chosen deliberately).

**v1's backend is functionally complete end-to-end and on GitHub.** Remaining open items: finish + verify the step 10 frontend end-to-end, decide on a free-tier Postgres host (Neon/Supabase) for anything beyond local Docker, and optionally a real Alembic baseline migration once that host exists.
