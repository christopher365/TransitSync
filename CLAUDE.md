...
12. ✅ Fix stale-prediction display bug + add live service alerts — added 2026-08-26. Fixed `minutesUntil()` unconditionally clamping any negative (past) time to 0, which made badly-delayed predictions display as "Arriving now" — root cause of a live bug report (a clicked "Arriving now" vehicle was miles from the stop). Also added a fourth MBTA data source, `GET /api/stops/{id}/alerts` (live, not persisted, same pattern as predictions), shown as a banner above predictions. Verified against real data: Back Bay currently has an active severity-7 Orange Line suspension, which plausibly explained the earlier bug report.

**v1 (backend + frontend, including stop search/predictions/alerts) is functionally complete end-to-end and on GitHub.** Remaining open items: decide on a free-tier Postgres host (Neon/Supabase) for anything beyond local Docker, and optionally a real Alembic baseline migration once that host exists.
